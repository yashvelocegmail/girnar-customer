...
function POGeneration() {
  ...
  //Customer option
  useEffect(() => {
    axios.post(api_url + "read_quotation_by_customer.php", { customer: localStorage.getItem('customer_id') })
      .then((res) => {
  ...
  const onFormSubmit = (e) => {
    ...
    axios.post(api_url + "create_purchase_order.php", purchaseOrder)
      .then(() => {
        console.log("Created")
        axios.post(api_url + "read_purchase_order_customer.php", { customer: localStorage.getItem("customer_id") })
          .then((res) => {
